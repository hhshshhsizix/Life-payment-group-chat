...
// 2. 定时发ping的槽函数
void Socket::sendPingTimer()
{
    // 构造ping的JSON
    QJsonObject pingObj;
    pingObj["type"] = "ping";
    pingObj["username"] = m_username;  // 可选：带上用户名，方便服务器识别
...
void Socket::onReplyFinished(QNetworkReply *reply)
{
    ...
    if(reply->error() != QNetworkReply::NoError){
        ...
        // }
        // 修复：只判断真正的 SSL 错误码，排除超时/拒绝等无关错误
        QSet<QNetworkReply::NetworkError> realSSLErrors = {
            QNetworkReply::SslHandshakeFailedError
        };
        if(realSSLErrors.contains(reply->error())){
            qDebug() << "ssl错误，检查证书是否有效/客户端是否缺少OpenSSL库";
        }
    ...
    if(parseError.error != QJsonParseError::NoError || !doc.isObject()){
        ...
        if(path == "/login"){
            ...
            if(replyObj.contains("status") && replyObj["status"].isString()){
                ...
                QString message = replyObj["message"].toString();
                if(status == "success"){
                    emit updateDeliveryStatusResult(true, message);
                }
                else{
...
void Socket::onTcpReadyRead()
{
    ...
    if (type == "group_msg") {
        ...
        bool isPrivate = false; // 群消息=非私聊，固定传false
        QString appPath = QCoreApplication::applicationDirPath();
        QDir cacheDir(appPath + "/cache");
        if (!cacheDir.exists()) cacheDir.mkdir("."); // 避免目录不存在报错
        // 检查本地有没有此人的头像
        QString imgPath = appPath + "/cache/avatar_" + from + ".png";
        QFileInfo fileInfo(imgPath);
        bool hasLocalAvatar = fileInfo.exists() && fileInfo.isFile();
        emit chatMessageReceived(from, text, time, isPrivate, hasLocalAvatar);
        // 没有，发送请求头像
        if (!hasLocalAvatar) {
        ...
        QString text = root["text"].toString();
        QString time = root["time"].toString();
        QString receiver = root["receiver"].toString();

        // 判断规则：receiver不是群标识 → 私聊（isPrivate=true）
        // 这里的"group"/"all"按你服务器的群标识调整
        bool isPrivate = (receiver != "group" && receiver != "all" && !receiver.startsWith("group_"));
        // 检查本地有没有此人的头像
        QString appPath = QCoreApplication::applicationDirPath();
        QString imgPath = appPath + "/cache/avatar_" + from + ".png";
        QFileInfo fileInfo(imgPath);
