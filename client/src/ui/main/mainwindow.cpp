    ...
    , m_info(userinfo)
{
    ...
    Socket::getInstance().ConnectChatServer(m_username, "192.168.142.131", 81); // 修改

    connect(&Socket::getInstance(), &Socket::chatMessageReceived, this, [=](QString from, QString text, QString time, bool isPrivate,bool hasLocalAvatar){
        AddChatMessage(from, text, time, isPrivate, hasLocalAvatar);
    });
...
void MainWindow::AddChatMessage(QString sender, QString content, QString time, bool isPrivate, bool hasLocalAvatar)
{
    // 在聊天室界面显示聊天信息
    ...
    if (isSelf) {
        ...
        QFileInfo selfAvatarFile(imagePath);
        // 自己的头像不存在时，也用默认头像（兜底）
        if (!selfAvatarFile.exists() || !selfAvatarFile.isFile()) {
            imagePath = ":/image/image/default_avatar.png"; // 统一用资源文件（推荐）
        }
        ...
        if (hasLocalAvatar) {
            // 有本地头像 → 用缓存的
            imagePath = appPath + "/cache/avatar_" + sender + ".png";
        } else {
            // 无本地头像 → 用默认头像（Socket层已经判断过，这里直接用）
    ...
    int itemWidth = ui->chatRoom_widget->viewport()->width() - 22;
    // 兜底：避免宽度过小（比如窗口缩太小时）
