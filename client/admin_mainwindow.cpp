    ...
    , ui(new Ui::Admin_MainWindow)
{
    ...
    connect(m_sock, &Socket::updateDeliveryStatusResult, this, [this](bool success, const QString& message){
        m_sock->SendGetProgressRequest("");
    });

    // 连接聊天服务器，因为要主动向服务器发送
    Socket::getInstance().ConnectChatServer("root", "192.168.142.131", 81); // 修改
...
void Admin_MainWindow::onallUserInfoResult(bool success, QString message, const QList<UserAllInfo> &list)
{
    if(success){
        ...
        qDeleteAll(ui->userInfo_Widget->findItems("", Qt::MatchContains));
